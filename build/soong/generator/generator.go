...
func init() {
	android.RegisterModuleType("naap_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = naapExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = naapExpandVariables(ctx, dep_file)
	...
	cmd := naapExpandVariables(ctx, String(g.properties.Cmd))
